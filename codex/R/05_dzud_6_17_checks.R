# Dedicated dzud exposure checks for age 6-17.
...
get_script_path <- function() {
  ...
  if (length(file_arg) == 0) return(file.path("codex", "R", "05_dzud_6_17_checks.R"))
  sub("^--file=", "", file_arg[[1]])
}
...
sink(file.path(codex_root, "output/logs/05_dzud_6_17_checks.log"), split = TRUE)
...
make_exposure_6_17 <- function(people, panel) {
  start_age <- 6L
  end_age <- 17L
  ...
  people %>%
    ...
    group_by(row_id) %>%
    summarise(
      n_complete_6_17 = sum(!is.na(loss_rate)),
      dzud_cum_6_17 = ifelse(n_complete_6_17 == expected_n, sum(loss_rate), NA_real_),
      dzud_max_6_17 = ifelse(n_complete_6_17 == expected_n, max(loss_rate), NA_real_),
      dzud_count5_6_17 = ifelse(n_complete_6_17 == expected_n, sum(dzud5 == 1, na.rm = TRUE), NA_real_),
      dzud_count10_6_17 = ifelse(n_complete_6_17 == expected_n, sum(dzud10 == 1, na.rm = TRUE), NA_real_),
      dzud_p75_count_6_17 = ifelse(n_complete_6_17 == expected_n, sum(dzud_p75 == 1, na.rm = TRUE), NA_real_),
      dzud_any_dzud5_6_17 = ifelse(n_complete_6_17 == expected_n, as.integer(sum(dzud5 == 1, na.rm = TRUE) > 0), NA_real_),
      .groups = "drop"
    )
}
...
hses <- hses %>% left_join(make_exposure_6_17(hses, dzud_panel), by = "row_id")
saveRDS(hses, file.path(codex_root, "data/cleaned/hses_dzud_exposure_6_17.rds"))
...
audit_6_17 <- tidyr::crossing(iv_var = iv_vars, fe_designs) %>%
  pmap_dfr(function(iv_var, fe_design, fe_formula, main) {
    run_model(hses, "wage_sample", iv_var, fe_design, fe_formula, main)
  })

write_csv(audit_6_17, file.path(codex_root, "output/tables/T2c_dzud_6_17_audit.csv"))
...
all_adults <- all_adults %>% left_join(make_exposure_6_17(all_adults, dzud_panel), by = "row_id")
...
logic_6_17 <- imap_dfr(sample_list, function(df, nm) {
  map_dfr(c("dzud_cum_6_17", "dzud_count5_6_17", "dzud_p75_count_6_17"), ~ run_first_stage_only(df, unique(df$sample_label)[1], .x))
})

coverage_6_17 <- bind_rows(
  hses %>% mutate(sample = "wage"),
  all_adults %>% mutate(sample = "all_adults")
) %>%
  mutate(complete_6_17 = !is.na(dzud_cum_6_17)) %>%
  group_by(sample, birth_cohort) %>%
  summarise(
    N = n(),
    complete_6_17_N = sum(complete_6_17),
    complete_6_17_share = mean(complete_6_17),
    .groups = "drop"
  )

write_csv(logic_6_17, file.path(codex_root, "output/tables/T2c_dzud_6_17_logic_checks.csv"))
write_csv(coverage_6_17, file.path(codex_root, "output/tables/T2c_dzud_6_17_coverage_by_cohort.csv"))

cat("Saved T2c_dzud_6_17_audit.csv rows:", nrow(audit_6_17), "\n")
cat("Saved T2c_dzud_6_17_logic_checks.csv rows:", nrow(logic_6_17), "\n")
cat("Saved T2c_dzud_6_17_coverage_by_cohort.csv rows:", nrow(coverage_6_17), "\n\n")
cat("Corrected main FE, wage sample:\n")
print(audit_6_17 %>% filter(fe_design == "birth_aimag + birth_cohort + wave") %>% arrange(desc(F_first)), n = 20)
cat("\nLogic checks:\n")
print(logic_6_17, n = nrow(logic_6_17))
