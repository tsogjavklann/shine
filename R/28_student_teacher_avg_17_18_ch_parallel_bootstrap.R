...
diff_plot <- ggplot(success, aes(x = 100 * beta_diff_boot)) +
  geom_histogram(bins = 35, fill = "#2f5d62", color = "white") +
  geom_vline(xintercept = 100 * beta_diff_observed, color = "#b33939", linewidth = 0.8) +
  ...
  labs(
    ...
    subtitle = paste0(bootstrap_inference$B_requested, " удаагийн бүүтстрап; 95% интервал [",
                      fmt(100 * bootstrap_inference$beta_diff_q025, 1),
                      "; ", fmt(100 * bootstrap_inference$beta_diff_q975, 1),
                      "] нэгж хувь; p = ", fmt(bootstrap_inference$bootstrap_p_value, 3))
  ) +
  theme_minimal(base_size = 11)
ggsave(
