# Stage 19E: Cluster bootstrap inference for log_distance_to_ub CH-style IV threshold model.
# This script does not search for IVs and does not use log_distance_to_ub as an IV.
...
log_path <- file.path(PATHS$out_logs, "19e_logdist_ch_bootstrap_inference.log")
sink(log_path, split = TRUE)
on.exit(sink(), add = TRUE)

cat("19e_logdist_ch_bootstrap_inference.R\n")
cat("Started:", as.character(Sys.time()), "\n\n")

set.seed(20260426)

data_path <- file.path(PATHS$data_proc, "ch_residualized_logdist_parent_mean.rds")
gamma_path <- file.path(PATHS$out_tables, "T8c_logdist_ch_gamma_hat.csv")
gmm_path <- file.path(PATHS$out_tables, "T8d_logdist_ch_gmm_final_results.csv")
wald_path <- file.path(PATHS$out_tables, "T8d_logdist_ch_gmm_wald_test.csv")
...
distance_observed <- as.numeric(gamma_tbl$distance_cutoff_km[1])
if (!is.finite(distance_observed)) {
  distance_observed <- ifelse(abs(gamma_observed) < .Machine$double.eps^0.5, 0, exp(gamma_observed))
}
...
make_design <- function(gamma, data) {
  low <- as.integer(data$log_distance_to_ub <= gamma)
  high <- as.integer(data$log_distance_to_ub > gamma)
...
estimate_2sls_at_gamma <- function(data, gamma) {
  ...
  k <- ncol(Xw)
  l <- ncol(Zw)
  min_regime_n <- max(30L, k + 1L)
  rank_X <- qr(Xw)$rank
  rank_Z <- qr(Zw)$rank
  if (dsgn$N_low < min_regime_n || dsgn$N_high < min_regime_n || rank_X < k || rank_Z < l) {
    return(NULL)
  }
  ...
  XPZX_inv <- safe_solve(XPZX)
  if (is.null(XPZX_inv)) return(NULL)
  beta <- as.numeric(XPZX_inv %*% XPZy)
  u <- as.numeric(y - X %*% beta)
  ssr <- sum(w * u^2, na.rm = TRUE)
  list(
    gamma = gamma,
    distance_cutoff_km = dsgn$distance_cutoff_km,
    beta = beta,
    SSR = ssr,
    N_low = dsgn$N_low,
    N_high = dsgn$N_high,
...
estimate_2sls_grid <- function(data, gamma_grid = NULL) {
  ...
  tbl <- bind_rows(lapply(fits[ok], function(obj) {
    tibble(
      gamma = obj$gamma,
      distance_cutoff_km = obj$distance_cutoff_km,
...
cat("Observed distance cutoff:", distance_observed, "\n")
...
for (b in seq_len(B)) {
  ...
  if (inherits(grid_fit, "error") || is.null(grid_fit)) {
    ...
    draws[[b]] <- tibble(
      b = b,
      gamma_boot = NA_real_,
      distance_cutoff_km_boot = NA_real_,
  ...
  if (inherits(gmm_fit, "error") || is.null(gmm_fit)) {
    ...
    draws[[b]] <- tibble(
      ...
      distance_cutoff_km_boot = grid_fit$distance_cutoff_km,
      beta_low_boot = NA_real_,
      beta_high_boot = NA_real_,
      beta_diff_boot = NA_real_,
      N_boot = nrow(boot),
...
write_csv(boot_draws, file.path(PATHS$out_tables, "T8e_logdist_ch_bootstrap_draws.csv"))
...
inference <- tibble(
  ...
  distance_cutoff_observed = distance_observed,
  distance_cutoff_q025 = qfun(success$distance_cutoff_km_boot, 0.025),
  distance_cutoff_q05 = qfun(success$distance_cutoff_km_boot, 0.05),
  distance_cutoff_q50 = qfun(success$distance_cutoff_km_boot, 0.50),
  distance_cutoff_q95 = qfun(success$distance_cutoff_km_boot, 0.95),
  distance_cutoff_q975 = qfun(success$distance_cutoff_km_boot, 0.975),
...
write_csv(inference, file.path(PATHS$out_tables, "T8e_logdist_ch_bootstrap_inference.csv"))
...
comparison <- tibble(
  ...
  conclusion = ifelse(
    ...
    "Bootstrap supports geographic threshold heterogeneity at 5%.",
    "Bootstrap does not strongly support geographic threshold heterogeneity at 5%."
  )
)
write_csv(comparison, file.path(PATHS$out_tables, "T8e_logdist_ch_asymptotic_vs_bootstrap.csv"))
...
gamma_plot <- ggplot(success, aes(x = gamma_boot)) +
  ...
  labs(
    x = "Bootstrap gamma",
    y = "Draws",
    title = "Stage 19E bootstrap distribution of log-distance gamma"
  ) +
  theme_minimal(base_size = 11)

ggsave(
  filename = file.path(PATHS$out_figures, "logdist_stage19e_gamma_bootstrap_distribution.png"),
  plot = gamma_plot,
  width = 7,
  height = 4.5,
  dpi = 300
...
diff_plot <- ggplot(success, aes(x = beta_diff_boot)) +
  ...
  labs(
    ...
    y = "Draws",
    title = "Stage 19E bootstrap distribution of log-distance regime difference"
  ) +
  theme_minimal(base_size = 11)

ggsave(
  filename = file.path(PATHS$out_figures, "logdist_stage19e_beta_diff_bootstrap_distribution.png"),
  plot = diff_plot,
  width = 7,
  height = 4.5,
  dpi = 300
...
interpretation <- if (heterogeneity_supported) {
  "Bootstrap inference supports education returns differing across geographic/labor-market-access regimes at the 5% level."
} else {
...
report_lines <- c(
  ...
  paste0("- Observed distance cutoff in km: ", fmt(distance_observed)),
  ...
  paste0("- distance cutoff 2.5% / 5% / 50% / 95% / 97.5%: ",
         fmt(inference$distance_cutoff_q025), " / ",
         fmt(inference$distance_cutoff_q05), " / ",
         fmt(inference$distance_cutoff_q50), " / ",
         fmt(inference$distance_cutoff_q95), " / ",
         fmt(inference$distance_cutoff_q975)),
  "",
  ...
  paste0("- Stage 19D asymptotic/Wald p-value: ", fmt(asymptotic_p)),
  paste0("- Stage 19E bootstrap p-value: ", fmt(p_boot)),
  "",
...
writeLines(report_lines, file.path(PATHS$out_root, "reports", "logdist_ch_stage19e_bootstrap_inference.md"), useBytes = TRUE)
