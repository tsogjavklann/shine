# Stage 19D: Caner-Hansen-style two-step GMM slope estimation at log_distance_to_ub gamma_hat.
# This script does not run bootstrap and does not use log_distance_to_ub as an IV.
...
log_path <- file.path(PATHS$out_logs, "19d_logdist_ch_gmm_slope.log")
sink(log_path, split = TRUE)
on.exit(sink(), add = TRUE)

cat("19d_logdist_ch_gmm_slope.R\n")
...
if (!file.exists(data_path)) stop("Missing ", data_path, ". Run Stage 19B first.")
if (!file.exists(gamma_path)) stop("Missing ", gamma_path, ". Run Stage 19C first.")
...
distance_cutoff_km <- ifelse(abs(gamma_hat) < .Machine$double.eps^0.5, 0, exp(gamma_hat))
...
df <- df |>
  mutate(
    low = as.integer(log_distance_to_ub <= gamma_hat),
    high = as.integer(log_distance_to_ub > gamma_hat)
  )
...
condition_ZtZ <- condition_number(ZtZ)
...
gmm_estimate <- function(W) {
  ...
  A_left_inv <- safe_solve(A_left)
  if (is.null(A_left_inv)) return(NULL)
  as.numeric(A_left_inv %*% A_right)
}
...
condition_S_robust <- condition_number(S_robust)
condition_S_cluster <- condition_number(S_cluster)
...
condition_XZWZX <- condition_number(B)
B_inv <- safe_solve(B)
...
near_singular_warning <- any(c(
  ...
  is.finite(condition_S_robust) && condition_S_robust > 1e10,
  is.finite(condition_S_cluster) && condition_S_cluster > 1e10
))
...
write_csv(matrix_diag, file.path(PATHS$out_tables, "T8d_logdist_ch_gmm_matrix_diagnostics.csv"))
write_csv(final_results, file.path(PATHS$out_tables, "T8d_logdist_ch_gmm_final_results.csv"))
write_csv(wald_test, file.path(PATHS$out_tables, "T8d_logdist_ch_gmm_wald_test.csv"))
write_csv(comparison, file.path(PATHS$out_tables, "T8d_logdist_ch_2sls_vs_gmm_comparison.csv"))

crit <- if (weighting_matrix_used == "cluster-robust S by birth_aimag") {
  qt(0.975, df = n_clusters - 1)
} else {
  qnorm(0.975)
}

low_label <- paste0("<= ", round(distance_cutoff_km, 1), " km")
high_label <- paste0("> ", round(distance_cutoff_km, 1), " km")
...
plot_data <- final_results |>
  mutate(
    regime = recode(
      term,
      educ_low = low_label,
      educ_high = high_label
    ),
    ci_low = estimate - crit * se,
    ci_high = estimate + crit * se,
    regime = factor(regime, levels = c(low_label, high_label))
  )
...
g <- ggplot(plot_data, aes(x = regime, y = estimate)) +
  ...
  labs(
    ...
    title = "Stage 19D log_distance_to_ub GMM regime-specific returns",
    subtitle = paste0("Threshold: log distance ", round(gamma_hat, 4),
                      " (", round(distance_cutoff_km, 1), " km)")
  ) +
  theme_minimal(base_size = 11)

ggsave(
  filename = file.path(PATHS$out_figures, "logdist_stage19d_gmm_regime_returns.png"),
  plot = g,
  width = 6,
  height = 4.5,
  dpi = 300
...
safe_stage19e <- all(c(rank_X, rank_Z, rank_ZtZ, rank_XZ, rank_XZWZX) == k) &&
  all(is.finite(final_results$estimate)) &&
  all(is.finite(final_results$se)) &&
  is.finite(wald_test$p_value)

decision <- if (safe_stage19e) {
  "Proceed to Stage 19E bootstrap inference."
} else {
  "Proceed to Stage 19E only with numerical caution; review matrix warnings first."
}
...
report_lines <- c(
  ...
  paste0("- distance cutoff in km: ", fmt(distance_cutoff_km)),
  "",
  "## 2. GMM Method",
  ...
  paste0("- rank(X'Z W Z'X): ", rank_XZWZX),
  paste0("- condition number Z'Z: ", fmt(condition_ZtZ)),
  paste0("- condition number S_robust: ", fmt(condition_S_robust)),
  paste0("- condition number S_cluster: ", fmt(condition_S_cluster)),
...
writeLines(report_lines, file.path(PATHS$out_root, "reports", "logdist_ch_stage19d_gmm_slope_estimation.md"), useBytes = TRUE)
